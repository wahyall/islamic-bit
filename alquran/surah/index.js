...
window.onload = function () {
  ...
  $.ajax({
    ...
    success: (results) => {
      ...
      expandDetail.forEach((expand) => {
        expand.addEventListener("click", function () {
          ...
          if (
            ...
            const infoHeight = getComputedStyle(
              this.parentElement.querySelector(".info")
            ).height;
            this.parentElement.querySelector(
              ".info-ayat"
            ).style.height = `calc(${infoHeight} + 2rem)`;
          } else {
      ...
      if (
        JSON.parse(localStorage.getItem("ISLAMIC BIT")).bacaanTerakhir &&
        JSON.parse(localStorage.getItem("ISLAMIC BIT")).bacaanTerakhir
          .nomerSurat == nomerSurat
      ) {
        const bacaanAyatTerakhir = JSON.parse(
          localStorage.getItem("ISLAMIC BIT")
        ).bacaanTerakhir.nomerAyat;
        ...
        $("html, body").animate({
          scrollTop:
            elementBacaanAyatTerakhir.offset().top -
            parseFloat(
              getComputedStyle(document.querySelector("nav")).height.replace(
                "px",
                ""
              )
            ) -
...
function autoPlay(index) {
  ...
  document
    .querySelectorAll(".ayat")
    [indexPlayedAudio].classList.add("current-play");
  const bacaanAyat = $(".current-play");
  $("html, body").animate({
    scrollTop:
      bacaanAyat.offset().top -
      parseFloat(
        getComputedStyle(document.querySelector("nav")).height.replace("px", "")
      ) -
      20,
  });
  ...
  const checkPlayedAudio = setInterval(() => {
    if (isPlayed) {
      if (audios[indexPlayedAudio].paused) {
        ...
        document
          .querySelectorAll(".ayat")
          [indexPlayedAudio].classList.remove("current-play");
